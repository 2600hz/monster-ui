define(function() {
	...
	function _addAppI18n(app) {
		...
		loadLocale(app, defaultLanguage, function() {
			...
			if (monster.config.whitelabel.language.toLowerCase() !== defaultLanguage.toLowerCase()) {
				loadLocale(app, monster.config.whitelabel.language, function() {
		...
		_.extend(app.i18n, {
			active: function() {
				var language = app.i18n.hasOwnProperty(monster.config.whitelabel.language) ? monster.config.whitelabel.language : defaultLanguage;
	...
	function _loadApp(name, callback, options) {
		var self = this,
			appPath = 'apps/' + name,
			customKey = 'app-' + name,
			requirePaths = {},
	...
	function changeAppShortcuts(app) {
	...
	function getActiveApp() {
		return lastLoadedApp;
	}

	function loadDependencies(app, globalCallback) {
		var listRequests = {},
	...
	function loadLocale(app, language, callback) {
		// Automatic upper case for text after the hyphen (example: change en-us to en-US)
		var language = language.replace(/-.*/, function(a) { return a.toUpperCase(); }),
		...
		if (app.i18n.hasOwnProperty(language)) {
			loadFile(function(data) {
				...
				if (language === defaultLanguage) {
					...
					app.data.i18n[language] = $.extend(true, app.data.i18n[language] || {}, app.data.i18n[defaultLanguage], data);
				}
	...
	function monsterizeApp(app, callback) {
		var self = this,
							...
							params.data.uploadProgress = function(progress) {
								if (progress.lengthComputable) {
									...
									if (progressValue === 100) {
										if (uploadProgress.active && progressBar.length) {
											progressBar
												.children('div')
													.width('99%')
													.html('99%');
											uploadProgress.amount--;
											if (uploadProgress.amount === 0) {
	...
	function load(name, callback, options, pChangeHash) {
		var changeHash = pChangeHash === true ? true : false,
			afterLoad = function(app) {
				lastLoadedApp = app.name;

				changeAppShortcuts(app);
