(function(winkstart, amplify, undefined) {
    ...
    $.each(default_languages, function(k, language) {
        ...
        $.ajax({
            url : 'config/home/locales/' + language +'/'+namespace+'.json',
            cache: false,
    ...
    amplify.module.constructor = function(args, callback) {
        ...
        if(this.config.templates) {
            ...
            if(!(THIS.__whapp in winkstart.templates)) {
                winkstart.templates[THIS.__whapp] = {};
            }
                ...
                if(THIS.__module + '/' + url in (winkstart.templates[THIS.__whapp] || {})) {
                    ...
                    $.ajax({
                        ...
                        success: function(template) {
                            ...
                            winkstart.templates[THIS.__whapp][THIS.__module + '/' + url] = THIS.templates[name];
                        }
                    });
                }
            });
        ...
        (function() {
            if ( completed == 0 ) {
                if ( $.isFunction(callback) ) {
                    ...
                    if(uri) {
                        console.log(THIS.__whapp, THIS.__module);
                        if(uri.tags.length == 1) {
                            if(THIS.__whapp == uri.tags[0] && THIS.__module == uri.tags[0]) {
                                winkstart.publish(uri.tags[0] + '.activate', {});
                            }
                        } else {
                            if(THIS.__whapp == uri.tags[0]) {
                                if(THIS.__module == uri.tags[1]) {
                                    winkstart.publish(uri.tags[0] + '.module_activate', {name: uri.tags[1]});
                                }
                            }
                        }
                    }*/
